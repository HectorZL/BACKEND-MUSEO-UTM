...
// --- ¡CAMBIO! Importar sirv-js ---
import multer from 'multer';
import Sirv from 'sirv-js'; // ¡Este es el nombre de paquete correcto!
...
/**
 * --- ¡CAMBIO! Nueva función helper para subir a Sirv ---
 */
const uploadToSirv = async (fileBuffer, originalname, folder) => {
  ...
  try {
    ...
    await sirvClient.auth();
    
    // Subir el archivo
    await sirvClient.uploadFile({
      data: fileBuffer,
      filename: sirvPath,
    });
    
    const publicUrl = `https://${sirvAccountName}.sirv.com${sirvPath}`;
    return publicUrl;
  } catch (error) {
    console.error("Error subiendo a Sirv:", error);
    if (error.response && error.response.data) {
      console.error("Detalles del error de Sirv:", error.response.data);
    }
